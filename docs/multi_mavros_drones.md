...
Each ardupilot SITL instance creates a couple of unique MAVlink streams. This is shown in your mavproxy terminal when you launch your ardupilot instance
```
SIM_VEHICLE: "mavproxy.py" "--master" "tcp:127.0.0.1:5770" "--sitl" "127.0.0.1:5511" "--out" "127.0.0.1:14560" "--out" "127.0.0.1:14561" "--console"
```
...
```
roslaunch iq_sim apm.launch fcu_url:=udp://127.0.0.1:14561@14565 mavros_ns:=/drone2
```
...
- you will notice that the `fcu_url` increments by 10 for each corresponding udp port fore the drones. 
- the `target_system_id` increments by 1 for each instance. 
- you will notice that each node has a unique ns (namespace). this will put a unique topi prefix for each mavros topic
